...
import awardsData from '@/data/mock/awards.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetAwards') || query.includes('nodeAwards')) {
      return loadMockData('awards.json')
    }
