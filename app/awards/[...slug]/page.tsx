...
import { GET_AWARD_BY_PATH } from '@/lib/queries'
import { DrupalAward } from '@/lib/types'
...
interface AwardByPathData {
  route: {
    entity: DrupalAward
  } | null
}

async function getAward(path: string): Promise<DrupalAward | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<AwardByPathData>({
      query: GET_AWARD_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/awards/${slug.join('/')}`
  const item = await getAward(path)

  if (!item) {
    return { title: 'Award Not Found | Architecture' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).awardingBody && (
                    <div>
                      <dt className="text-sm text-gray-500">Awarding Body</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).awardingBody}</dd>
                    </div>
                  )}
                  {(item as any).awardYear && (
                    <div>
                      <dt className="text-sm text-gray-500">Award Year</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).awardYear}</dd>
                    </div>
                  )}
                  {(item as any).projectName && (
                    <div>
                      <dt className="text-sm text-gray-500">Project Name</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).projectName}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
