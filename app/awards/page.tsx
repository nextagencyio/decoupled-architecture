import { getClient } from '@/lib/drupal-client'
import { Metadata } from 'next'
import { GET_AWARDS } from '@/lib/queries'
import { AwardsData } from '@/lib/types'
import Header from '../components/Header'
import AwardCard from '../components/AwardCard'

export const revalidate = 3600
export const dynamic = 'force-dynamic'

export const metadata: Metadata = {
  title: 'Awards | Arden & Cole Architects',
  description: 'Browse our awards.',
}

async function getAwards() {
  try {
    const client = getClient()
    const data = await client.raw(GET_AWARDS, { first: 50 })
    return data?.nodeAwards?.nodes || []
  } catch (error) {
    console.error('Error fetching awards:', error)
    return []
  }
}

export default async function AwardsPage() {
  const items = await getAwards()

  return (
    <div className="min-h-screen bg-primary-950">
      <Header />

      <section className="relative py-24 md:py-32">
        <div className="absolute inset-0 bg-gradient-to-b from-primary-900/50 to-primary-950" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center pt-16">
            <div className="flex justify-center mb-6">
              <div className="w-12 h-px bg-gradient-to-r from-transparent via-stone-400 to-transparent" />
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-light text-white mb-6">
              Awards
            </h1>
            <p className="text-xl text-stone-400 max-w-3xl mx-auto">
              Explore our awards and recognition.
            </p>
          </div>
        </div>
      </section>

      <section className="py-16 md:py-20 bg-primary-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            <div className="text-center py-12">
              <h2 className="text-2xl font-semibold text-stone-400 mb-2">No Awards Yet</h2>
              <p className="text-stone-500">
                Awards will appear here once content is imported.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {items.map((item: any) => (
                <AwardCard key={item.id} item={item} />
              ))}
            </div>
          )}
        </div>
      </section>
    </div>
  )
}
